...
try:
    from eval_sim_pushcube.analysis_dp.mr_rules import MR_RULE_REGISTRY
except Exception:
    # 兼容直接运行该脚本（python eval_sim_pushcube/analysis_dp/mr_eval_analyzer_dp.py）
    from eval_sim_pushcube.analysis_dp.mr_rules.registry import MR_RULE_REGISTRY
...
def _default_output_path(mr_id: str) -> str:
    ...
    return os.path.join(base_dir, f"PushCube_dp_{safe_mr_id}_{ts}.json")
