...
GDIP1_SIMILARITY_THRESHOLD = 0.85
GDIP1_MIN_ALIGNMENT_POINTS = 4
GDIP1_SAMPLE_POINTS = 32
...
@register_mr_rule("GDIP-Full-Temporal-Vision-Blackout")
def analyze_mr_gdip1_full_temporal_blackout(
    base_records: List[Any], mr_records: List[Any], **kwargs
) -> Dict[str, Any]:
    similarity_threshold = float(kwargs.get("similarity_threshold", GDIP1_SIMILARITY_THRESHOLD))
    ...
    for key in keys:
        ...
        if base_points and mr_points:
            ...
            if metrics is None:
                ...
                if similarity > similarity_threshold:
                    violated = True
                    reasons.append(
                        "VIOLATION: Full-Temporal Vision Blackout Breakdown "
                        f"(similarity={similarity:.6f} > {similarity_threshold:.6f})"
                    )
                else:
                    reasons.append("trajectory_diverged_under_blackout")
        else:
            analyzable = False
        ...
        if bool(mr_success):
            violated = True
            reasons.append("VIOLATION: mr_success_true_under_full_blackout")

        if not analyzable:
            unavailable_count += 1
            violated = bool(mr_success)
        elif violated:
            violations += 1
    ...
    return {
        ...
        "config": {
            "similarity_threshold": similarity_threshold,
            "sample_points": sample_points,
            "invariance_proxy": "full_temporal_blackout_trajectory_consistency",
            "attack_type": "vision_full_temporal_blackout",
        },
        "details": details,
    }
