...
class EpisodeRecord:
    ...
    eef_vel: List[Optional[List[float]]]
    gripper_action_cmd: List[Optional[float]]
    tcp_to_obj_distance: List[Optional[float]]
    is_contact: List[Optional[bool]]
    joint_torques: List[Optional[List[float]]]
...
def load_records(traj_dir: str) -> List[EpisodeRecord]:
    ...
    for fp in files:
        ...
        # Some MRs (e.g., LTSEP1) expect an explicit target_pos trajectory.
        # Prefer traj['target_pos'] when present; fall back to dst_cube_pos for backward compatibility.
        target_pos = _normalize_xyz_sequence(_first_not_none(traj.get("target_pos"), dst_cube_pos) or [])
        ...
        eef_vel = _normalize_xyz_sequence(traj.get("eef_vel", []) or [])
        gripper_action_cmd = traj.get("gripper_action_cmd", []) or []
        tcp_to_obj_distance = traj.get("tcp_to_obj_distance", []) or []
        is_contact = traj.get("is_contact", []) or []
        joint_torques = traj.get("joint_torques", []) or []
        ...
        rec = EpisodeRecord(
            ...
            tcp_to_obj_distance=[None if x is None else float(x) for x in tcp_to_obj_distance],
            is_contact=[None if x is None else bool(x) for x in is_contact],
            joint_torques=[
                None if item is None else [float(v) for v in np.asarray(item, dtype=np.float64).reshape(-1).tolist()]
...
try:
    from eval_sim_pushcube.analysis_RDT.mr_rules import MR_RULE_REGISTRY
except Exception:
    # 兼容直接运行该脚本（python eval_sim_pushcube/analysis_RDT/mr_eval_analyzer_rdt.py）
    from eval_sim_pushcube.analysis_RDT.mr_rules.registry import MR_RULE_REGISTRY
...
def _normalize_mr_id(mr_id: str) -> str:
    ...
    aliases = {
        "MR-SESP-1": "MR-SESP1",
        "MR-LTSEP-1": "MR-LTSEP1",
        "mr-sesp-1": "mr_sesp_1",
        "mr-sesp1": "mr_sesp_1",
        "mr_sesp1": "mr_sesp_1",
        "mr-ltsep-1": "mr_ltsep_1",
        "mr-ltsep1": "mr_ltsep_1",
        "mr_ltsep1": "mr_ltsep_1",
    }
    if raw in aliases:
        return aliases[raw]
    return raw
...
def _default_output_path(mr_id: str) -> str:
    ...
    return os.path.join(base_dir, f"PushCube_rdt_{safe_mr_id}_{ts}.json")


def main():
    parser = argparse.ArgumentParser(description="Analyze eval_rdt trajectory JSONs for MR evaluation.")
