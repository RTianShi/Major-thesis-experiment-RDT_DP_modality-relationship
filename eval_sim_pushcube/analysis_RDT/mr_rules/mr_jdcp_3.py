...
JDCP3_SIMILARITY_THRESHOLD = 0.90
JDCP3_MIN_ALIGNMENT_POINTS = 4
JDCP3_SAMPLE_POINTS = 32
...
def _trajectory_similarity_metrics(
    ...
    *,
    ...
    if sample_count < JDCP3_MIN_ALIGNMENT_POINTS:
        return None
...
@register_mr_rule("MR-JDCP3")
@register_mr_rule("MR-JDCP-3")
@register_mr_rule("mr_jdcp_3")
def analyze_mr_jdcp3_deprivative_vision_mutation(
    base_records: List[Any], mr_records: List[Any], **kwargs
) -> Dict[str, Any]:
    similarity_threshold = float(kwargs.get("similarity_threshold", JDCP3_SIMILARITY_THRESHOLD))
    sample_points = int(kwargs.get("sample_points", JDCP3_SAMPLE_POINTS))
        ...
        if analyzable and base_points and mr_points:
            ...
            if metrics is None:
                ...
                if similarity > similarity_threshold:
                    violated = True
                    reasons.append(
                        "VIOLATION: Open-loop Execution (Blind trajectory is a rigid copy of the sighted one) "
                        f"(similarity={similarity:.6f} > {similarity_threshold:.6f})"
                    )
                else:
                    reasons.append("trajectory_diverged_under_vision_deprivation")
    ...
    return {
        ...
        "config": {
            ...
            "violation_rule": "trajectory_similarity > threshold",
        },
        "details": details,
    }
