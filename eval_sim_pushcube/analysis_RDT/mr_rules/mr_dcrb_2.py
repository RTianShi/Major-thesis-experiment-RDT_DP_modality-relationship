...
DCRB1_SIMILARITY_THRESHOLD = 0.82
DCRB1_MIN_ALIGNMENT_POINTS = 4
DCRB1_SAMPLE_POINTS = 32
...
def _trajectory_similarity_metrics(
    ...
    *,
    ...
    if sample_count < DCRB1_MIN_ALIGNMENT_POINTS:
        return None
...
@register_mr_rule("MR-DCRB2")
@register_mr_rule("MR-DCRB-2")
@register_mr_rule("mr_dcrb_2")
def analyze_mr_dcrb2_ood_rebinding(
    base_records: List[Any], mr_records: List[Any], **kwargs
) -> Dict[str, Any]:
    similarity_threshold = float(kwargs.get("similarity_threshold", DCRB1_SIMILARITY_THRESHOLD))
    sample_points = int(kwargs.get("sample_points", DCRB1_SAMPLE_POINTS))
        ...
        if analyzable and base_points and mr_points:
            ...
            if metrics is None:
                ...
                if similarity >= similarity_threshold:
                    violated = True
                    reasons.append(
                        "VIOLATION: Trajectory Similarity Too High "
                        f"(similarity={similarity:.6f} >= {similarity_threshold:.6f})"
                    )
                else:
                    reasons.append("trajectory_diverged_under_ood_rebinding")
