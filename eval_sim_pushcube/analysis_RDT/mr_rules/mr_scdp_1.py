...
SCDP1_SIMILARITY_THRESHOLD = 0.90
SCDP1_MIN_ALIGNMENT_POINTS = 4
SCDP1_SAMPLE_POINTS = 32
...
def _trajectory_similarity_metrics(
    ...
    *,
    ...
    if sample_count < SCDP1_MIN_ALIGNMENT_POINTS:
        return None
...
def analyze_mr_scdp1_visual_background_debunking(
    base_records: List[Any], mr_records: List[Any], **kwargs
) -> Dict[str, Any]:
    similarity_threshold = float(kwargs.get("similarity_threshold", SCDP1_SIMILARITY_THRESHOLD))
    sample_points = int(kwargs.get("sample_points", SCDP1_SAMPLE_POINTS))
        ...
        if analyzable and base_points and mr_points:
            ...
            if metrics is None:
                ...
                if similarity < similarity_threshold:
                    violated = True
                    reasons.append(
                        "VIOLATION: Trajectory Strong Consistency Breakdown "
                        f"(similarity={similarity:.6f} < {similarity_threshold:.6f})"
                    )
                else:
                    reasons.append("trajectory_consistent_under_background_change")
    ...
    return {
        ...
        "config": {
            "similarity_threshold": similarity_threshold,
            "sample_points": sample_points,
            "invariance_proxy": "strict_trajectory_similarity_under_background_change",
            "attack_type": "table_texture_lighting_and_out_of_workspace_distractors",
        },
        "details": details,
    }
