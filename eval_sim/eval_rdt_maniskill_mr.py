...
for episode in tqdm.trange(total_episodes):
    ...
    if args.save_video and video_frames and (
        ...
        writer = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(*"mp4v"), args.video_fps, (int(target_w), int(target_h)))
        for frame in video_frames:
            if (frame.shape[1], frame.shape[0]) != (int(target_w), int(target_h)):
                resized = cv2.resize(frame, (int(target_w), int(target_h)), interpolation=cv2.INTER_LINEAR)
            else:
                resized = frame
