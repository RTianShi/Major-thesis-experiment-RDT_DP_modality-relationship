...
def sample_actions(
    ...
    actions = actions[0] # 取出第一批
    
    # --- 核心：反归一化 ---
    # 使用训练时保存的数据集统计信息，把 [-1, 1] 转回真实关节弧度
    from octo.utils.spec import ActionProprioSpec
    actions = pretrained_model.dataset_statistics['action']['unnormalize'](actions)
    # remove batch dim
    return actions
...
# 必须手动覆盖，因为加载 Checkpoint 不一定会自动覆盖这些动态配置
model.config["model"]["heads"]["action"]["kwargs"]["action_dim"] = 8
model.config["model"]["heads"]["action"]["kwargs"]["proprio_dim"] = 9
model.config["model"]["heads"]["action"]["kwargs"]["action_horizon"] = 4
...
for episode in tqdm.trange(total_episodes):
    ...
    while global_steps < MAX_EPISODE_STEPS and not done:
        obs = {
            ...
            # --- 必须补齐的字段 ---
            "timestep": np.array([global_steps, global_steps], dtype=np.int32), 
            "task_completed": np.zeros((2,), dtype=bool),
            "pad_mask_dict": {
                "proprio": np.ones((2,), dtype=bool),
                "image_primary": np.ones((2,), dtype=bool),
                "timestep": np.ones((2,), dtype=bool),
            }
        }        
        ...
        if args.show:
            show_img = img
            if show_img.shape[-1] == 3:
                cv2.imshow("octo_eval", cv2.cvtColor(show_img, cv2.COLOR_RGB2BGR))
                key = cv2.waitKey(1)
                if key == 27:  # ESC退出
                    exit(0)
