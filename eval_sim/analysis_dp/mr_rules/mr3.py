...
@register_mr_rule("mr3")
def analyze_mr3_visual_perturbation(
        ...
        if analyzable and base_points and mr_points:
            ...
            if metrics is None:
                ...
                if similarity <= similarity_threshold:
                    violated = True
                    reasons.append(
                        "VIOLATION: Visual perturbation changed trajectory too much "
                        f"(similarity={similarity:.6f} <= {similarity_threshold:.6f})"
                    )
                else:
                    reasons.append("trajectory_stable_under_visual_perturbation")
