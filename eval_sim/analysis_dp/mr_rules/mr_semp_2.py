...
def analyze_mr_semp_2(base_records: List[Any], mr_records: List[Any], **kwargs) -> Dict[str, Any]:
    """
    MR-SEMP-2: 时空刚体平移等变测试
    - 衍生用例将方块和目标整体平移 delta_d = (0.04, -0.04, 0.0)
    - 仅根据抓取点和平移终点的实际位移误差判定是否违反 MR
    - 若实际位移偏离 delta_d 超过位置容差，则判定为空间等变性破坏
    """
    delta_d = np.asarray(
        kwargs.get("translation_delta", [0.04, -0.04, 0.0]),
        dtype=np.float64,
    ).reshape(-1)
    if delta_d.size < 3:
    ...
    for k in keys:
        ...
        if reasons:
            ...
            if grasp_error > pos_tol:
                violated = True
                reasons.append(f"Spatial_Anchor_Drift_At_Grasp (Error: {grasp_error:.3f}m)")
            if end_error > pos_tol:
                violated = True
                reasons.append(f"Spatial_Anchor_Drift_At_End (Error: {end_error:.3f}m)")
