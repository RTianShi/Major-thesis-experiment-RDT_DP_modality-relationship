...
def _env_np_rng(env):
    """Return an episode-seeded RNG if available.

    In DP eval we call env.reset(seed=...), and Gymnasium-style envs expose
    `np_random` seeded by that call. Prefer it over global `np.random` so env
    mutations are reproducible per episode.
    """
...
def _resolve_scalar_or_range(value, *, rng, default: float) -> float:
    """Resolve either a scalar or a [low, high] range into a float.

    This keeps the original config surface (dx/dy) while allowing DP-friendly,
    per-episode randomized translations when users pass a 2-element list/tuple.
    """
    if value is None:
...
def _ensure_mr2_visual_asset(env, cfg):
    ...
    if runtime is not None and runtime.get("scene") is getattr(env.unwrapped, "scene", None):
        return runtime
...
def _iter_collision_shapes(actor):
    if actor is None:
        return
    for attr in ("get_collision_shapes", "collision_shapes", "_collision_shapes"):
        shapes = getattr(actor, attr, None)
        if callable(shapes):
            try:
                shapes = shapes()
            except Exception:
...
def _make_physical_material(scene, cfg):
    creator = getattr(scene, "create_physical_material", None)
    if creator is None:
        return None
    static_friction = float(cfg.get("static_friction", cfg.get("friction", 1.2)))
    dynamic_friction = float(cfg.get("dynamic_friction", static_friction))
    restitution = float(cfg.get("restitution", 0.0))
    try:
        return creator(static_friction, dynamic_friction, restitution)
    except Exception:
        try:
            return creator(
...
def _set_actor_physical_friction(actor, cfg):
    scene = getattr(getattr(actor, "scene", None), "scene", None)
    if scene is None:
        scene = getattr(cfg.get("env", None), "unwrapped", None)
        scene = getattr(scene, "scene", None) if scene is not None else None
    if scene is None:
        return

    physical_material = _make_physical_material(scene, cfg)
    if physical_material is None:
        return

    for shape in _iter_collision_shapes(actor):
        for method_name in ("set_physical_material", "set_material"):
            method = getattr(shape, method_name, None)
            if callable(method):
                try:
                    method(physical_material)
                    break
                except Exception:
                    pass
        for attr_name in ("physical_material", "material"):
            try:
                setattr(shape, attr_name, physical_material)
                break
            except Exception:
                pass
...
def env_mr_sesp2_surface_friction_force_synergy(env, cfg):
    ...
    # 当前 DP：尽量同时改物理摩擦和视觉外观
    _set_actor_physical_friction(red_cube, cfg)
...
def env_mr_semp1_global_translation_equivariance(env, cfg):
    ...
    rng = _env_np_rng(env)
    dx = _resolve_scalar_or_range(cfg.get("dx", 0.04), rng=rng, default=0.04)
    dy = _resolve_scalar_or_range(cfg.get("dy", 0.0), rng=rng, default=0.0)
...
def env_mr4_target_object_relocation(env, cfg):
    ...
    rng = _env_np_rng(env)
    dx = _resolve_scalar_or_range(cfg.get("dx", 0.03), rng=rng, default=0.03)
    dy = _resolve_scalar_or_range(cfg.get("dy", 0.0), rng=rng, default=0.0)
    if abs(dx) < 1e-8 and abs(dy) < 1e-8:
        return env
...
def env_mr_semp2_z_axis_rotation_equivariance(env, cfg):
    ...
    rng = _env_np_rng(env)
    delta_theta_deg = _resolve_scalar_or_range(
        cfg.get("delta_theta_deg", 45.0),
        rng=rng,
        default=45.0,
    )
    if abs(delta_theta_deg) < 1e-8:
        return env
