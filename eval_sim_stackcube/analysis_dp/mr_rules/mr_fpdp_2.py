...
FPDP2_TRAJECTORY_SIMILARITY_THRESHOLD = 0.95
FPDP2_MIN_ALIGNMENT_POINTS = 4
FPDP2_SAMPLE_POINTS = 24
...
def _trajectory_similarity_metrics(
    lowres_points: List[List[float]],
    base_points: List[List[float]],
    *,
    sample_points: int,
) -> Optional[Dict[str, float]]:
    if lowres_points is None or base_points is None:
        return None

    sample_count = min(int(sample_points), len(lowres_points), len(base_points))
    if sample_count < FPDP2_MIN_ALIGNMENT_POINTS:
        return None

    lowres_resampled = _resample_polyline(lowres_points, sample_count)
    base_resampled = _resample_polyline(base_points, sample_count)
    if lowres_resampled is None or base_resampled is None:
        return None

    pointwise_dist = np.linalg.norm(lowres_resampled - base_resampled, axis=1)
    mean_pointwise_deviation = float(pointwise_dist.mean())

    lowres_path_len = _path_length(lowres_points)
    base_path_len = _path_length(base_points)
    path_scale = base_path_len if base_path_len is not None and base_path_len > 1e-12 else lowres_path_len
    ...
    endpoint_deviation = float(np.linalg.norm(lowres_resampled[-1] - base_resampled[-1]))

    return {
        "mean_pointwise_deviation_m": mean_pointwise_deviation,
        "lowres_path_len_m": float(lowres_path_len) if lowres_path_len is not None else None,
...
def _analyze_fpdp2_spatial_downsampling(
    ...
    *,
        ...
        if analyzable:
            ...
            if metrics is None:
                ...
                if similarity > similarity_threshold:
                    violated = True
                    reasons.append(
                        "VIOLATION: Zero-Feedback Execution (Trajectory remains rigid at low resolution)."
                    )
                else:
                    reasons.append("expected_precision_loss_under_spatial_downsampling")
        ...
        details.append(
            {
                ...
                "lowres_path_len_m": None if metrics is None else metrics["lowres_path_len_m"],
    ...
    return {
        ...
        "config": {
            "sample_points": sample_points,
            "similarity_threshold": similarity_threshold,
            "metric": "eef_trajectory_similarity_under_spatial_downsampling",
            "violation_rule": "trajectory_similarity > similarity_threshold",
            "invariance_expectation": "low_resolution_should_reduce_closed_loop_precision",
        },
        "details": details,
    }
...
@register_mr_rule("mr_fpdp_2")
def analyze_mr_fpdp2_spatial_downsampling(
    base_records: List[Any], mr_records: List[Any], **kwargs
) -> Dict[str, Any]:
    return _analyze_fpdp2_spatial_downsampling(
        base_records,
        mr_records,
        mr_id=kwargs.get("mr_id", "MR-FPDP2-SPATIAL-DOWNSAMPLING"),
        similarity_threshold=float(
            kwargs.get("similarity_threshold", FPDP2_TRAJECTORY_SIMILARITY_THRESHOLD)
        ),
        sample_points=int(kwargs.get("sample_points", FPDP2_SAMPLE_POINTS)),
    )
