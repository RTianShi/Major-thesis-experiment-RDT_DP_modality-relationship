...
SCDP2_SIMILARITY_THRESHOLD = 0.9
SCDP2_MIN_ALIGNMENT_POINTS = 4
SCDP2_SAMPLE_POINTS = 24
...
def _trajectory_similarity_metrics(
    ...
    *,
    ...
    if sample_count < SCDP2_MIN_ALIGNMENT_POINTS:
        return None
...
def _analyze_scdp2_proprioceptive_noise_debunking(
    ...
    *,
        ...
        if analyzable:
            ...
            if metrics is None:
                ...
                if similarity < similarity_threshold:
                    violated = True
                    reasons.append("VIOLATION: EEF trajectory distorted by redundant joint noise")
                else:
        ...
        details.append(
            {
                ...
                "mr_path_len_m": None if metrics is None else metrics["mr_path_len_m"],
    ...
    return {
        ...
        "config": {
            "sample_points": sample_points,
            "similarity_threshold": similarity_threshold,
            "metric": "eef_trajectory_similarity",
            "violation_rule": "trajectory_similarity < similarity_threshold",
            "invariance_expectation": "redundant_joint_noise_should_not_distort_eef_trajectory",
        },
        "details": details,
    }
...
def analyze_mr_scdp2_proprioceptive_noise_debunking(
    base_records: List[Any], mr_records: List[Any], **kwargs
) -> Dict[str, Any]:
    return _analyze_scdp2_proprioceptive_noise_debunking(
        base_records,
        mr_records,
        mr_id=kwargs.get("mr_id", "MR-SCDP2-PROPRIOCEPTIVE-NOISE-DEBUNKING"),
        similarity_threshold=float(kwargs.get("similarity_threshold", SCDP2_SIMILARITY_THRESHOLD)),
        sample_points=int(kwargs.get("sample_points", SCDP2_SAMPLE_POINTS)),
    )
