...
def _analyze_b1_color_comutate(
    ...
    *,
    ...
    for k in keys:
        ...
        if blue_cube is None:
            analyzable = False
            reasons.append("missing_blue_cube_position")

        dist = _point_l2(grasp_point, blue_cube)
        if analyzable:
            if dist is None:
                analyzable = False
                ...
                violated = True
                reasons.append(f"grasp_far_from_blue_cube({dist:.4f} > {dist_thresh:.4f})")
            else:
                reasons.append("grasp_near_blue_cube")
    ...
    return {
        ...
        "config": {
            "distance_thresh_m": dist_thresh,
            "metric": "dist(Pgrasp_f, Pblue_cube)",
...
@register_mr_rule("MR-B1")
@register_mr_rule("MR-B1-COMUTATE-COLOR")
def analyze_mr_b1_color_comutate(base_records: List[Any], mr_records: List[Any], **kwargs) -> Dict[str, Any]:
    return _analyze_b1_color_comutate(
        base_records,
        mr_records,
        mr_id=kwargs.get("mr_id", "MR-B1"),
        dist_thresh=float(kwargs.get("dist_thresh_m", MR_B1_GRASP_DIST_THRESH)),
    )
