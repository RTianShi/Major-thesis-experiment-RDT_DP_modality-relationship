...
def _point_l2(a: Optional[np.ndarray], b: Optional[np.ndarray]) -> Optional[float]:
...
def _translation_vector(base: Any, mr: Any) -> Optional[np.ndarray]:
    ...
    if base_green is not None and mr_green is not None:
        delta = mr_green - base_green
        delta[2] = 0.0
        return delta
    return None
...
def _analyze_semp1_global_translation_equivariance(
    ...
    *,
    ...
    for k in keys:
        ...
        final_drift_m = None
        if analyzable:
            grasp_drift_m = _translation_error(
                binfo["grasp_point"],
                minfo["grasp_point"],
                translation_vec,
            )
            final_drift_m = _translation_error(
                binfo["final_point"],
                minfo["final_point"],
                translation_vec,
            )
            ...
            if grasp_drift_m is None or final_drift_m is None:
                analyzable = False
                reasons.append("cannot_compute_equivariance_drifts")
            else:
                if grasp_drift_m > position_tol_m:
                    violated = True
                    reasons.append("VIOLATION: Grasp anchor spatial drift (Equivalence failure)")
                if final_drift_m > position_tol_m:
                    violated = True
                    reasons.append("VIOLATION: Final placement spatial drift (Equivalence failure)")
                if not violated:
                    reasons.append("global_translation_equivariance_pass")
        ...
        details.append(
            {
                ...
                "grasp_translation_error_m": grasp_drift_m,
                "final_translation_error_m": final_drift_m,
    ...
    return {
        ...
        "config": {
            ...
            "violation_rule": "translation_error(grasp) > tol OR translation_error(final) > tol",
            "invariance_expectation": "global_scene_translation_should_equivariantly_shift_action_anchors",
        },
        "details": details,
    }
...
def analyze_mr_semp1_global_translation_equivariance(
    base_records: List[Any], mr_records: List[Any], **kwargs
) -> Dict[str, Any]:
    return _analyze_semp1_global_translation_equivariance(
        base_records,
        mr_records,
        mr_id=kwargs.get("mr_id", "MR-SEMP1-GLOBAL-TRANSLATION-EQUIVARIANCE"),
        position_tol_m=float(kwargs.get("position_tol_m", SEMP1_POSITION_TOL_M)),
    )
