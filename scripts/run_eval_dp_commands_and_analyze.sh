...
# 4. 命令里可以不写 --traj-dir。脚本会按 eval_rdt_maniskill_mr.py 的默认命名规则：
#    eef_traj/PickCube/<env-id>_<mr-type>_<timestamp>
#    自动找到本次运行后最新生成的目录。

#第一处要修改的位置，执行评估命令，修改为当前MR所需的场景，--mr-type是保存的MR文件名
COMMANDS=(
  "python -m eval_sim.eval_rdt_maniskill_mr --pretrained_path ./mp_rank_00_model_states.pt -e PickCube-v1 --show"
  "python -m eval_sim.eval_rdt_maniskill_mr --pretrained_path ./mp_rank_00_model_states.pt -e PickCubeWoodTable-v1 --show "
)
