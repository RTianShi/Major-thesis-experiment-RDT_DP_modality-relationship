...
class RoboticDiffusionTransformerModel(object):
    ...
    def __init__(
        ...
        self.device = device
        self.text_encoder_device = text_encoder_device or device
        self.text_model_dtype = torch.float32 if self.text_encoder_device == "cpu" else self.dtype
    ...
    def get_text_encoder(self, pretrained_text_encoder_name_or_path):
        text_embedder = T5Embedder(
            ...
            torch_dtype=self.text_model_dtype,
    ...
    def reset(self):
        ...
        self.text_model = self.text_model.to(self.text_encoder_device, dtype=self.text_model_dtype)
    ...
    def load_pretrained_weights(self, pretrained=None):
        ...
        if os.path.isdir(pretrained):
            for candidate in ["model.safetensors", "pytorch_model.bin", "pytorch_model.pt"]:
                candidate_path = os.path.join(pretrained, candidate)
                if os.path.exists(candidate_path):
                    pretrained = candidate_path
                    print(f'Found checkpoint file: {pretrained}')
                    break
        ...
        if filename.endswith('.pt') or filename.endswith('.bin'):
            ...
            if isinstance(checkpoint, dict):
                ...
                raise ValueError(f"Unexpected checkpoint type for {pretrained}: {type(checkpoint)}")
            ...
            if missing_keys:
                print(f"Missing keys when loading policy: {len(missing_keys)}")
            if unexpected_keys:
                print(f"Unexpected keys when loading policy: {len(unexpected_keys)}")
