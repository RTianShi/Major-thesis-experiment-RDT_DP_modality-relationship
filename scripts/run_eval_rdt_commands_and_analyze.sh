...
#    eef_traj/stackCube/<env-id>_<mr-type>_<timestamp>
#    自动找到本次运行后最新生成的目录。

#第一处要修改的位置，执行评估命令，修改为当前MR所需的场景，--mr-type是保存的MR文件名
COMMANDS=(
  "python -m eval_sim.eval_rdt_maniskill_mr --pretrained_path ./mp_rank_00_model_states.pt -e PickCubeSceneTrans04N04-v1 --show"
  "python -m eval_sim.eval_rdt_maniskill_mr --pretrained_path ./mp_rank_00_model_states.pt -e PickCubeSceneTrans04N04-v1 --show "  
)
...
for i in "${!COMMANDS[@]}"; do
  ...
  if [[ "${#MR_CONFIGS_INLINE[@]}" -gt 0 ]]; then
    ...
    if [[ -n "$mr_cfg_inline" ]]; then
      tmp_yaml="$(mktemp /tmp/eval_rdt_inline_mr_XXXXXX.yaml)"
